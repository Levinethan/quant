...
CYBOTRADE_ORDER_QUANTITY = 0.031
EXCHANGE = Exchange.BinanceLinear
start_time = datetime(2023, 5, 10, 0, 0, 0, tzinfo=timezone.utc)
end_time = datetime(2025, 3, 16, 0, 0, 0, tzinfo=timezone.utc)

class Strategy(BaseStrategy):
    symbol = [Symbol(base="BTC", quote="USDT")]
    sma_length = 50
    z_score_threshold = 0.75
    quantity = float(CYBOTRADE_ORDER_QUANTITY)
    ...
    async def on_candle_closed(self, strategy, topic, symbol):
        ...
        if z_score > self.z_score_threshold:
            if current_pos.long.quantity == 0.0:
                try:
                    await strategy.open(exchange=EXCHANGE,side=OrderSide.Buy, quantity=self.quantity, symbol=symbol, limit=None, take_profit=None, stop_loss=None, is_hedge_mode=False, is_post_only=False)
                except Exception as e:
                    logging.error(f"Failed to open long: {e}")
        else:
            if current_pos.long.quantity != 0.0:
                try:
                    await strategy.close(exchange=EXCHANGE, side=OrderSide.Buy, quantity=current_pos.long.quantity, symbol=symbol, is_hedge_mode=False)
                except Exception as e:
...
config = RuntimeConfig(
            ...
            candle_topics=["binance-linear|candle?symbol=BTCUSDT&interval=1d"],
            # ---------------------- change endpoint ----------------------
