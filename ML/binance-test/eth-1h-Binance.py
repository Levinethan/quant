...
init_position_from_db = True
place_order_exchange = Exchange.BinanceLinear

TELEGRAM_CHAT_ID = "-4232729820"
TELEGRAM_TOKEN = "bot7078735113:AAHONKAYEPfOJcVC3SABNgT1t-9IVPc6EVc"
...
class Strategy(BaseStrategy):
    ...
    bot_sr = 2.3
    leverage = 1.5
    bot_id = "eth_coinbase_premium_index_gap_threshold_1h"
    parameter_json_id = "sr_table"
    multiplier = 0.15
    rolling_window = 320
    db_params = {
        "database": "binance-eth",
        "user": "liang",
        "password": "qwer1234",
        "host": "localhost",
        "port": "5432",
    ...
    def __init__(self):
        ...
        file_handler = TimedRotatingFileHandler(
            "./logs/y_coinbase_pi_gap_1h_threshold-livetestnet.log", when="h", backupCount=30
        )
        ...
        #)
        self.leverage = 1.5
        self.renew_sr_table_time = datetime.now(pytz.timezone("UTC"))
    ...
    async def on_datasource_interval(self, strategy, topic, data_list):
        self.position = await strategy.position(symbol=self.pair,exchange=Exchange.BinanceLinear)
        logging.info(f"activate_order_interval long position:{self.position.long}, short position:{self.position.short}")
        ...
        if (
            ...
            self.position = Position(
                ...
                updated_time = int(datetime.now(pytz.timezone("UTC")).timestamp() * 1000)
            )
...
config = RuntimeConfig(
    mode=runtime_mode,
    datasource_topics=[
        'cryptoquant|eth/market-data/coinbase-premium-index?window=hour'
    ],
    active_order_interval=60,
    initial_capital=1000000.0,
    candle_topics=["binance-linear|candle?symbol=ETHUSDT&interval=1h"],
...
hyper_parameters = {}
hyper_parameters["rolling_window"] = [300]
hyper_parameters["multiplier"] = [0.23]
